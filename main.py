...
def index():
  ...
  conn = sqlite3.connect('flask.db')
  cursor = conn.cursor()
  cursor.execute('SELECT BLOGID,TITLE,CATEGORY,DESCRIPTION,DATE,IMAGE FROM dotchainblogtable WHERE OBJECTIVE=(?) ORDER BY DATE DESC LIMIT (?)',('lost',6))
  commercial=cursor.fetchall()
  cursor.execute('SELECT BLOGID,TITLE,CATEGORY,DESCRIPTION,DATE,IMAGE FROM dotchainblogtable WHERE OBJECTIVE=(?) ORDER BY DATE DESC LIMIT (?)',('found',12))
  ...
  cursor.execute('SELECT BLOGID,TITLE,CATEGORY,DATE,IMAGE FROM dotchainblogtable WHERE OBJECTIVE=(?) ORDER BY DATE DESC LIMIT (?)',('lost',10))
...
def staffdashboard():
  ...
  if request.method=='POST':
    ...
    if (username == "jaishriram" and userpass == "jaishriram"):
...
def blogupdate(blogid,username):
  if('user'in session and  session['user'] == "jaishriram"):
    ...
    categories = ['electronics','money','books','clothes']  # Add more categories as needed in lower
    objectives = ['lost','found']  # Add more objectives as needed in lower
    try:
      if request.method=='POST':
        image=request.files['image']
        if image.filename!="":
